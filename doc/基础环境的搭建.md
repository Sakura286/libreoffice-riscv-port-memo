...
> 参考：[Ubuntu、Debian 系列安装 Docker](http://shouce.jb51.net/docker_practice/install/ubuntu.html)
...
> 参考1：[Docker Buildx](https://docs.docker.com/buildx/working-with-buildx/)
> 参考2：
...
下载地址：[Index of linux/debian/dists/bullseye/pool/test/amd64/](https://download.docker.com/linux/debian/dists/bullseye/pool/test/amd64/)

下载并安装`containerd.io`、`docker-ce-cli`、`docker-ce`三个包，注意先后顺序

此文使用的是当时最新的稳定版

- containerd.io_1.6.6-1_amd64.deb
- docker-ce-cli_20.10.9~3-0~debian-bullseye_amd64.deb
- docker-ce_20.10.9~3-0~debian-bullseye_amd64.deb

### 2. 非 root 使用 Docker

参考资料：[Manage Docker as a non-root user](https://docs.docker.com/engine/install/linux-postinstall/)
...
测试命令`docker run hello-world`，如果没有提示权限错误，则前面操作生效了

### 3. 复制仓库到本地

```shell
git clone https://github.com/yuzibo/LibreOffice-riscv-port
cd LibreOffice-riscv-port
```
